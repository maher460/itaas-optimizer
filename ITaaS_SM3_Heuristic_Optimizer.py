from src.service_model_2_pr_heuristic_optimizer import *
...
sm2_functions_to_test = [sm2_pr_closest_min_sites, 
...
for num_apps in apps_collection.keys():
    for list_num in range(len(apps_collection[num_apps])):
        ...
        for func in sm2_functions_to_test:
            ...
            (placements, pr_groups, sites_machines) = func(app_list, aws_locations_short)

            end_time = time.time()

            execution_time = (end_time - start_time) * 1000

            quality = calc_quality_service_model_2(app_list, placements, aws_locations_short, True)
            quality["algo_name"] = func.__name__
            quality["time"] = str(round(execution_time, 2))

            temp_res.append(quality)

        fname = "./data/benchmark_results/service_model_2_proactive_recovery_heuristic/res_app_list_"+str(num_apps)+"_"+str(list_num)+".csv"
