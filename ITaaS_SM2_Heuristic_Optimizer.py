from src.service_model_2_heuristic_optimizer import *
...
sm2_functions_to_test = [sm2_round_robin_min_sites, 
...
for num_apps in apps_collection.keys():
    for list_num in range(len(apps_collection[num_apps])):
        ...
        for func in sm2_functions_to_test:
            ...
            quality = calc_quality_service_model_2(app_list, res, aws_locations_short, False)
            quality["algo_name"] = func.__name__
            quality["time"] = str(round(execution_time, 2))

            temp_res.append(quality)

        fname = "./data/benchmark_results/service_model_2_heuristic/res_app_list_"+str(num_apps)+"_"+str(list_num)+".csv"
