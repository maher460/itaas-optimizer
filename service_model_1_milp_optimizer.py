...
def sm1_milp_max_sites(app_charcs, resources):
    ...
    #max_num_replicas = apps*sites*servers
    ...
    #r = model.add_var(name="R", lb=0, ub=0.9999999, var_type=CONTINUOUS)
    ...
    for i in range(len(app_charcs)):
        ...
        for cur_sites in range(min_sites, max_sites+1):
            ...
            max_replicas_per_site = math.ceil(min_replicas/cur_sites)
            ...
            for (l_site, s1_site, s2_site, s3_site) in product(range(sites), range(sites), range(sites), range(sites)):
                model += (s[app][l_site] * lats_apps[app][l_site] * 2) \
                     ...
                     <= app_charcs[app][3] * a[app][conf] + 1000 * (1-a[app][conf]) #<= app_charcs[app][3] * xsum(a[app][conf] for conf in range(num_confs)) + 1000 * (1-xsum(a[app][conf] for conf in range(num_confs)))
    ...
    # model += xsum(a[app][conf] for (app,conf) in product(range(apps),range(num_confs))) >= 5
    # model += xsum(a[app][conf] for (app,conf) in product(range(apps),range(num_confs))) <= 5
    ...
    #     num_replicas += int(x[app][site].x)

    # print("/n/n DONE WITH TWOOOOO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! /n/n")
    # print(num_replicas)

    # model += xsum(x[app][site] for (app,site) in product(range(apps),range(sites))) == num_replicas

    # model.objective = minimize(xsum( (s[app][l_site] * lats_apps[app][l_site] * 2 + z[app][l_site][s1_site] * lats_sites[l_site][s1_site] + z[app][s2_site][s3_site] * lats_sites[s2_site][s3_site] * 2) / app_charcs[app][3] for (app, l_site, s1_site, s2_site, s3_site) in product(range(apps), range(sites), range(sites), range(sites), range(sites))) / (apps * sites * sites * sites * sites))

    # model.optimize()

    # End of Minimizing lat



    # model2 = Model()

    # #c = model2.add_var(name="C", var_type=CONTINUOUS)
    # r = model2.add_var(name="R", lb=0, ub=0.9999999, var_type=CONTINUOUS)
    # x = [[model2.add_var(var_type=INTEGER, lb=0, name='x({},{})'.format(app+1, site+1)) for site in range(sites)] for app in range(apps)]
    # a = [[model2.add_var(var_type=BINARY, name='a({},{})'.format(app+1, conf+1)) for conf in range(num_confs)] for app in range(apps)]
    # s = [[model2.add_var(var_type=BINARY, name='s({},{})'.format(app+1, site+1)) for site in range(sites)] for app in range(apps)]
    # z = [[[model2.add_var(var_type=BINARY, name='z({},{},{})'.format(app+1, site1+1, site2+1)) for site2 in range(sites)] for site1 in range(sites)] for app in range(apps)]
    ...
    #     model2 += xsum(a[app][conf] for conf in range(num_confs)) <= 1

    #     for temp_conf in range(min_sites-1):
    #         #print(temp_conf)
    #         model2 += a[app][temp_conf] == 0
    #     for temp_conf in range(max_sites, sites):
    #         #print(temp_conf)
    #         model2 += a[app][temp_conf] == 0
    ...
    #         model2 += xsum(s[app][site] for site in range(sites)) <= a[app][conf] * cur_sites + (1-a[app][conf]) * sites
    #         model2 += xsum(s[app][site] for site in range(sites)) >= a[app][conf] * cur_sites
    ...
    #             # matching s (site) with x (placement) (s is 1 when any x in that site is 1, s is 0 otherwise)
    #             for site in range(sites):
    #                 model2 += x[app][site] >= s[app][site]
    #             for site in range(sites):
    #                 model2 += x[app][site] <= s[app][site] * max_replicas_per_site

    #             # z is 1 if both s1 and s2 is 1, z is 0 otherwise
    #             for (site1, site2) in product(range(sites), range(sites)):
    #                 model2 += z[app][site1][site2] <= s[app][site1]
    #                 model2 += z[app][site1][site2] <= s[app][site2]
    #                 model2 += z[app][site1][site2] >= s[app][site1] + s[app][site2] - 1

    #             # lat constraint
    #             for (l_site, s1_site, s2_site, s3_site) in product(range(sites), range(sites), range(sites), range(sites)):
    #                 model2 += ((s[app][l_site] * lats_apps[app][l_site] * 2) \
    #                      + (z[app][l_site][s1_site] * lats_sites[l_site][s1_site]) \
    #                      + (z[app][s2_site][s3_site] * lats_sites[s2_site][s3_site] * 2) \
    #                      <= app_charcs[app][3])

    #         #Each app has at least the minimum number of replicas
    #         model2 += xsum(x[app][site] for site in range(sites)) >= min_replicas * a[app][conf] #(PHASE 2: CAN MULTIPLY THE CONFIGURATION VARIABLE HERE? c[app][config]?)
    #         model2 += xsum(x[app][site] for site in range(sites)) <= min_replicas * a[app][conf] + (1-a[app][conf]) * max_num_replicas

    #         # multi site (# PHASE 2 and also PHASE 1: We can replace this with by just requiring sum(s[app][site]) to be the required number of sites for this app and configuration)
    #         # for site in range(sites):
    #         #     model2 += x[app][site] <= max_replicas_per_site * a[app][conf] + servers * (1-a[app][conf]) #(PHASE 2: Maybe decrease this max_replicas_per_site to force more multiple sites for the different configs? Multiply this with c[app][config] so only one is true)

    # # At most 4 replicas permachine
    # # for site in range(sites):
    # #     model2 += xsum(x[app][site] for app in range(apps)) <= servers * 4

    # #model2 += c >= (xsum( (s[app][l_site] * lats_apps[app][l_site] * 2 + z[app][l_site][s1_site] * lats_sites[l_site][s1_site] + z[app][s2_site][s3_site] * lats_sites[s2_site][s3_site] * 2) / app_charcs[app][3] for (app, l_site, s1_site, s2_site, s3_site) in product(range(apps), range(sites), range(sites), range(sites), range(sites))) / (apps * sites * sites * sites * sites))

    # model2 += xsum(a[app][conf] for (app,conf) in product(range(apps),range(num_confs))) == num_apps

    # model2 += r >= xsum(x[app][site] for (app,site) in product(range(apps),range(sites))) / float(max_num_replicas)

    # model2.objective = minimize(r)
