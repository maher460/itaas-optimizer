from src.service_model_2_pr_milp_optimizer_v1 import *
...
sm2_functions_to_test = [sm2_pr_milp_max_sites]
...
for num_apps in apps_collection.keys():
    ...
    for list_num in list_num_list: #range(len(apps_collection[num_apps])):
        ...
        for func in sm2_functions_to_test:
            ...
            quality = calc_quality_service_model_2(app_list, res, aws_locations_short, True)
            quality["algo_name"] = func.__name__
            quality["time"] = str(round(execution_time, 2))

            temp_res.append(quality)

        fname = "./data/benchmark_results/service_model_2_proactive_recovery_milp/res_app_list_"+str(num_apps)+"_"+str(list_num)+".csv"
