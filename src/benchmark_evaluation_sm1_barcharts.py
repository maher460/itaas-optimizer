...
#cost_per_vm = 145.27 #c4.xlarge
cost_per_vm = 290.54 #c4.2xlarge
...
def run():
    ...
    #fig_cost.suptitle("Service Model 1: Average Cost (lower is better)")
    axs_cost = fig_cost.subplots(3, 1, sharex=True, sharey=True)

    fig_cost_per_app = plt.figure(figsize=(7, 8), layout="constrained")
    #fig_cost_per_app.suptitle("Service Model 1: Average Cost per App (lower is better)")
    ...
    df_cost['Avg'] = pd.to_numeric(df_cost['Avg']) * cost_per_vm
    df_cost['Min'] = pd.to_numeric(df_cost['Min']) * cost_per_vm
    df_cost['Max'] = pd.to_numeric(df_cost['Max']) * cost_per_vm
    ...
    df_cost_per_app['Avg'] = pd.to_numeric(df_cost_per_app['Avg']) * cost_per_vm / df_cost_per_app['number_of_apps']
    df_cost_per_app['Min'] = pd.to_numeric(df_cost_per_app['Min']) * cost_per_vm / df_cost_per_app['number_of_apps']
    df_cost_per_app['Max'] = pd.to_numeric(df_cost_per_app['Max']) * cost_per_vm / df_cost_per_app['number_of_apps']
    ...
    plt.savefig('./evaluation_results/SM1_VerifiedPlacements_Avg_Bar.png', bbox_inches='tight')
    plt.close()

    plt.savefig('./evaluation_results/SM1_ExecutionTimes_Avg_Bar.png', bbox_inches='tight')
    plt.close()

    plt.savefig('./evaluation_results/SM1_CostPerApp_Avg_Bar.png', bbox_inches='tight')
    plt.close()

    plt.savefig('./evaluation_results/SM1_Cost_Avg_Bar.png', bbox_inches='tight')
    plt.close()

    plt.savefig('./evaluation_results/SM1_Replicas_Avg_Bar.png', bbox_inches='tight')
    plt.close()

    plt.savefig('./evaluation_results/SM1_Apps_Avg_Bar.png', bbox_inches='tight')
    plt.close()
